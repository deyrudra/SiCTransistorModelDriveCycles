...
from unity.unity_world_streamer import UnityWorldStreamer
...
class TrafficVisualizer:
    def __init__(self, network: RoadNetwork, simulation: Simulation,
        ...
        # Mirror the 2D camera's streamed area into Unity. This owns the TCP
        # UnityBridge, so do not run unity_bridge.py as a separate process.
    ...
    def run(self) -> None:
        while self.running:
            ...
            # The Pygame camera is authoritative for world streaming. As it
            # pans across the 250 m grid, the matching GLB region is streamed
            # to Unity.
            self.unity_streamer.update(
